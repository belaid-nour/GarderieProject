...
export class LecturesComponent implements OnInit, OnDestroy {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => ({
      ...
      Time: x.time,
      Status: x.status,
      'Teacher ID': x.teacher_id,
      'Subject ID': x.subject_id,
      'Student Group': x.student_group,
      'Duration (Minutes)': x.duration,
      Location: x.location,
      'Attendance Count': x.attendance_count,
      'Created At': x.created_at,
      'Updated At': x.updated_at,
      'Reason for Cancellation': x.reason_for_cancellation,
    }));

    TableExportUtil.exportToExcel(exportData, 'class_session_export');
  }
